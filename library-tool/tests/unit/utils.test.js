...
describe('Languages module — Contract', () => {
  ...
  it('should export an availableLanguages array', async () => {
    ...
    assert.ok(availableLanguages.length > 0, 'Should contain at least one language');
  });
  ...
  it('every entry should have { name: string, value: string }', async () => {
    ...
    for (const lang of availableLanguages) {
      assert.strictEqual(typeof lang.name, 'string', `name should be string, got ${typeof lang.name}`);
      assert.strictEqual(typeof lang.value, 'string', `value should be string, got ${typeof lang.value}`);
  ...
  it('language values should be unique', async () => {
    ...
    assert.strictEqual(unique.size, values.length, 'All language codes should be unique');
  });
...
describe('Logger module — Contract', () => {
  ...
  it('should export a logger with standard methods', async () => {
    ...
    for (const method of requiredMethods) {
      assert.strictEqual(typeof logger[method], 'function', `logger.${method} should be a function`);
    }
  });
