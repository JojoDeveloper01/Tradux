...
describe("Config Integration", () => {
  before(async () => {
    ...
    await writeFile(
      join(testDir, "public/i18n/es.json"),
      JSON.stringify(
        {
          greeting: "Hola",
  ...
  describe("tradux.config.json", () => {
    ...
    it("should contain the required keys", async () => {
      const config = JSON.parse(
        await readFile(join(testDir, "tradux.config.json"), "utf8"),
      );
  ...
  describe("Language JSON files", () => {
    it("default language file should exist and be valid JSON", async () => {
      ...
      assert.ok(
        ...
        "Language file should not be empty",
      );
    });
    ...
    it("translated files should have the same top-level keys as default", async () => {
      ...
      const es = JSON.parse(
        await readFile(join(testDir, "public/i18n/es.json"), "utf8"),
      );
      ...
      assert.deepStrictEqual(
        esKeys,
        enKeys,
        "Translated file should mirror the default structure",
      );
    });
  ...
  describe(".env file", () => {
    it("should contain the required Cloudflare variables", async () => {
      ...
      assert.ok(
        ...
        "Should have CLOUDFLARE_ACCOUNT_ID",
      );
      assert.ok(
        env.includes("CLOUDFLARE_API_TOKEN"),
        "Should have CLOUDFLARE_API_TOKEN",
      );
    });
  });
});
