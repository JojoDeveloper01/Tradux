...
console.warn = (m) => { if (typeof m === 'string' && !m.includes('Tradux')) origWarn(m); };
console.error = (m) => { if (typeof m === 'string' && !m.includes('Failed to')) origError(m); };
...
describe('Core Functionality', () => {
  ...
  before(async () => {
    ...
    await writeFile(join(testDir, 'public/i18n/en.json'), JSON.stringify(translations, null, 2));
  ...
  describe('FileManager', () => {
    ...
    it('exists() should return true for files that exist', async () => {
      const { fileManager } = await import('../../src/core/file-manager.js');
      assert.strictEqual(fileManager.exists(join(testDir, 'tradux.config.json')), true);
      assert.strictEqual(fileManager.exists(join(testDir, 'nonexistent.json')), false);
    });

    it('loadLanguageFile() should return parsed JSON for a valid file', async () => {
      const { fileManager } = await import('../../src/core/file-manager.js');
      const data = await fileManager.loadLanguageFile(join(testDir, 'public/i18n'), 'en');
    ...
    it('loadLanguageFile() should return null for missing language', async () => {
      const { fileManager } = await import('../../src/core/file-manager.js');
      const data = await fileManager.loadLanguageFile(join(testDir, 'public/i18n'), 'xyz');
  ...
  describe('Translation file structure', () => {
    it('all leaf values should be strings', async () => {
      const data = JSON.parse(await readFile(join(testDir, 'public/i18n/en.json'), 'utf8'));
      ...
      function assertAllStrings(obj, path = '') {
        for (const [key, value] of Object.entries(obj)) {
          ...
          if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
            ...
            assert.strictEqual(typeof value, 'string', `Value at "${p}" should be a string`);
          }
        }
      }
    ...
    it('nested keys should be dot-accessible', async () => {
      ...
      for (const dotPath of paths) {
        ...
        assert.strictEqual(typeof current, 'string', `"${dotPath}" should resolve to a string`);
      }
    });
  });
  ...
  describe('Error handling', () => {
    it('should handle corrupt JSON gracefully', async () => {
      await writeFile(join(testDir, 'public/i18n/corrupt.json'), '{invalid json!!!}');
      const { fileManager } = await import('../../src/core/file-manager.js');
      const result = await fileManager.loadLanguageFile(join(testDir, 'public/i18n'), 'corrupt');
