...
process.chdir(showcaseDir)

const routeConfigs = [
  {
    name: 'react-vite',
    prefix: '/react-vite/',
    directory: path.join(frameworksDir, 'react-vite'),
    distDirectory: path.join(frameworksDir, 'react-vite', 'dist'),
    dev: {
      port: 5173,
      command: 'pnpm',
      args: ['dev', '--host', '0.0.0.0', '--port', '5173', '--strictPort'],
      env: {
        TRADUX_EXAMPLES_BASE: '/react-vite/',
      },
    },
  },
  {
    name: 'vue-vite',
    prefix: '/vue-vite/',
    directory: path.join(frameworksDir, 'vue-vite'),
    distDirectory: path.join(frameworksDir, 'vue-vite', 'dist'),
    dev: {
      port: 5174,
      command: 'pnpm',
      args: ['dev', '--host', '0.0.0.0', '--port', '5174', '--strictPort'],
      env: {
        TRADUX_EXAMPLES_BASE: '/vue-vite/',
      },
    },
  },
  {
    name: 'svelte-vite',
    prefix: '/svelte-vite/',
    directory: path.join(frameworksDir, 'svelte-vite'),
    distDirectory: path.join(frameworksDir, 'svelte-vite', 'dist'),
    dev: {
      port: 5175,
      command: 'pnpm',
      args: ['dev', '--host', '0.0.0.0', '--port', '5175', '--strictPort'],
      env: {
        TRADUX_EXAMPLES_BASE: '/svelte-vite/',
      },
    },
  },
  {
    name: 'vanilla-vite',
    prefix: '/vanilla-vite/',
    directory: path.join(frameworksDir, 'vanilla-vite'),
    distDirectory: path.join(frameworksDir, 'vanilla-vite', 'dist'),
    dev: {
      port: 5176,
      command: 'pnpm',
      args: ['dev', '--host', '0.0.0.0', '--port', '5176', '--strictPort'],
      env: {
        TRADUX_EXAMPLES_BASE: '/vanilla-vite/',
      },
    },
  },
  {
...
function serveRootAsset(requestPath, response) {
  if (requestPath.startsWith('/i18n/')) {
    return sendFile(response, path.join(showcaseDir, requestPath.slice(1)))
  }
  ...
  const filePath = path.join(showcaseDir, requestPath.slice(1))
...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    sendHtml(
      response,
      503,
      `<h1>${route.name} is not built</h1><p>Run <code>pnpm build</code> inside <code>examples/showcase/</code> before starting production mode.</p>`,
    )
  } catch (error) {
    console.error(error)
